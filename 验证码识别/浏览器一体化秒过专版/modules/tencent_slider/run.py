...
腾讯/通用滑块 独立运行入口
用法：
  python run.py <URL> [--headless] [--wait 180]
"""
import argparse
import sys
import time
...
from .solver import TencentSliderSolver, visible, _log


def run(url, model="qwen3-vl:4b", submit=None, headless=False, wait_sec=180, storage_state=None):
    solver = TencentSliderSolver(model=model)
    _log(f"[Slider] 启动浏览器访问: {url}")
    ...
    with sync_playwright() as pw:
        ...
        try:
            ...
            while time.monotonic() < deadline:
                ...
                _log(f"[Slider] 状态: {state}")
                if state == "widget_passed":
                    _log("[Slider] 滑块已成功对齐并校验通过！")
                    if submit:
                        btn = visible(page, submit)
                        if btn:
                            _log(f"[Slider] 点击提交按钮: {submit}")
                            btn.click()
                    return True
                page.wait_for_timeout(1000)
            _log("[Slider] 超时未完成滑块验证")
            return False
        finally:
...
def main():
    parser = argparse.ArgumentParser(description="腾讯滑块与通用滑块拼图 独立识别工具")
    parser.add_argument("url", help="包含滑块验证码的目标网页地址")
